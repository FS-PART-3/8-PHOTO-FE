...
const SIZE_CLS = { '62':'text-[62px]', '48':'text-[48px]', '46':'text-[46px]', '40':'text-[40px]', '32':'text-[32px]', '28':'text-[28px]', '24':'text-[24px]', '22':'text-[22px]' };
...
function TitleButton({ label, href, size = 'L', variant = 'primary' }) {
  ...
  return href
    ? <Link href={href} className={`${base} ${dims} ${color}`}>{inner}</Link>
...
export default function Title({
  ...
  const justifyCls = justify === 'between' ? 'justify-between' : justify === 'center' ? 'justify-center' : 'justify-start';
  const alignCls = align === 'end' ? 'items-end' : align === 'center' ? 'items-center' : 'items-start';

  const widthStyle = containerWidth === 'full'
    ? { width:'100%' }
    : { width: typeof containerWidth === 'number' ? `${containerWidth}px` : '100%' };

  const familyVar = family === 'secondary' ? 'var(--font-secondary)' : 'var(--font-primary)';

  return (
    <div className={`inline-flex w-full flex-col ${gapCls}`}>
      <div className={`inline-flex ${justifyCls} ${alignCls}`} style={widthStyle}>
        <h1 className={`${colorClass} ${sizeCls} leading-none break-words`} style={{ fontFamily: familyVar, fontWeight: weight }}>
          {text}
        </h1>
        {action ? (
          <TitleButton
